<template>
  <div class="row" style="margin-top:15px">
    <div class="form-group">
      <input id="add-label" v-model="labelAdded" type="checkbox">
      <label for="add-label">
        Add Label
      </label>
      <input v-if="labelAdded === true" id="component-label" v-model="label" class="form-control form-control-sm"
        type="text">
    </div>
    <div class="form-group">
      <label for="pyb-answer">
        Answer field name | <span class="label-tip">The field where the worker's answer is stored. Can be JSON path like
          a.b.c.</span>
      </label>
      <input id="pyb-answer" v-model="pybAnswer" class="form-control form-control-sm" type="text">
    </div>
    <div class="form-group">
      <label for="editMode">
        Edit mode
      </label>
      <select id="editMode" v-model="editMode" class="form-control" :class="{ 'danger-validation': getErrors(`readOnly`) }">
        <option selected value="readWrite">
    ...
    <div id="tags" class="scroll">
      <div v-for="(tag, index) in tagList" :key="index">
        <hr v-if="index != 0" size="100px">
        <label>Tag {{ index + 1 }}</label>
        <button v-if="tagList.length > 1" class="btn btn-times-delete pull-right fa fa-times"
          :title="`Delete Tag ${index + 1}`" @click="deleteTag(index)" /><br>
        <label class="block-label">
          Name | <span class="label-tip">The tag name to store in the answer.</span>
          <select v-model="tag.name" class="form-control form-control-sm"
        ...
        <label class="block-label">
          ...
          <input v-model="tag.display" class="form-control form-control-sm"
            :class="{ 'danger-validation': getErrors(`tagList[${index}].display`) }" type="text">
        </label>
        <div class="danger-validation-text">
        ...
        <label class="block-label">
          ...
          <input v-model="tag.color" class="form-control form-control-sm"
            :class="{ 'danger-validation': getErrors(`tagList[${index}].color`) }" type="color">
        </label>
        <div class="danger-validation-text">
    ...
    <button id="add" class="btn btn-default btn-sm col-sm-2" @click="addTag">
      Add Another Tag
    </button>
    <br>
    <div class="form-group">
      ...
      <input v-model="confidenceThreshold" class="form-control form-control-sm" type="number" min="0" max="1" step="0.01">
      <div class="danger-validation-text">
    ...
    <div>
      ...
      <div>
        <label class="col-labels right-padding-radio">
          <input v-model="sourceType" value="variable" type="radio">
          From Variable
        </label>
        <label class="col-labels">
          <input v-model="sourceType" value="static" type="radio">
          Static
        </label>
      </div>
      <div v-if="sourceType === 'variable'" class="form-group">
        <input id="useStatic" v-model="useStaticInPreview" style="vertical-align:top" type="checkbox">
        <label class="col-labels" for="useStatic">
          Use static in preview.
        </label>
        <br>
        <label for="useStatic" class="label-tip">
    ...
    <div>
      <label class="block-label">
        ...
        <input id="textSource" v-model="textSource" type="text" class="form-control form-control-sm"
    ...
    <template v-if="sourceType === 'variable'">
      <label class="block-label">
        Entities Variable | <span class="label-tip">A JavaScript expression that returns the entities. For example,
          task.info.entities.</span>
        <input v-model="entitySource" type="text" class="form-control form-control-sm"
    ...
    <template v-else-if="sourceType === 'static'">
      <label class="col-labels" :class="{ 'danger-validation-text': getErrors(`entities.static`) }">
        {{ entityList.length ? "Entities" : "No Entities" }}
      </label>
      <div class="danger-validation-text">
        {{ getErrors(`entities.static`) }}
      </div>
      <div id="entities" class="scroll">
        <div v-for="(entity, index) in entityList" :key="index">
          <hr v-if="index != 0" size="100px">
          <label>Entity {{ index + 1 }}</label>
          <button class="btn btn-times-delete pull-right fa fa-times" :title="`Delete Entity ${index + 1}`"
            @click="deleteEntity(index)" /><br>
          <label class="block-label">
            Head Offset | <span class="label-tip">The character position where the entity starts. Count starts with
              0.</span>
            <input v-model.number="entity.headoffset" class="form-control form-control-sm"
              :class="{ 'danger-validation': getErrors(`entities.static[${index}].headoffset`) }" type="text">
          </label>
          <div class="danger-validation-text">
            {{ getErrors(`entities.static[${index}].headoffset`) }}
          </div>
          <label class="block-label">
            Tail Offset | <span class="label-tip">The character position AFTER the entity ends. Count starts with
              0.</span>
            <input v-model.number="entity.tailoffset" class="form-control form-control-sm"
              :class="{ 'danger-validation': getErrors(`entities.static[${index}].tailoffset`) }" type="text">
          </label>
          <div class="danger-validation-text">
          ...
          <label class="block-label">
            ...
            <select v-model="entity.taggedtype"
              :class="{ 'danger-validation': getErrors(`entities.static[${index}].taggedtype`) }"
              class="form-control form-control-sm">
              <option v-for="tagName in tagNames" :key="tagName" :value="tagName">
                {{ tagName }}
              </option>
            </select>
          </label>
      ...
      <button id="addEntity" class="btn btn-default btn-sm col-sm-2 col-md-2" @click="addEntity">
