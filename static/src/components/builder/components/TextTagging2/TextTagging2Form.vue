...
export default {
  ...
  computed: {
    ...
    useStaticInPreview: {
      get () {
        return this.$store.state.textTagging2.useStaticInPreview;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_USE_STATIC_IN_PREVIEW, newValue);
      }
    },
    tagNames: {
      get () {
    ...
    entitySource: {
      get () {
        return this.$store.state.textTagging2.entities.variable;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_ENTITY_SOURCE, newValue);
      }
    },
    sourceType: {
      get () {
        return this.$store.state.textTagging2.sourceType;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_SOURCE_TYPE, newValue);
      }
    },
    textSource: {
      get () {
        return this.$store.state.textTagging2.text[this.sourceType];
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_TEXT, newValue);
      }
    },
    editMode: {
      get () {
        return this.$store.state.textTagging2.readOnly ? 'readOnly' : 'readWrite';
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_READONLY, newValue === 'readOnly');
      }
    },
    confidenceThreshold: {
      get () {
        return this.$store.state.textTagging2.confidenceThreshold;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_CONFIDENCE_THRESHOLD, newValue);
      }
    },
    pybAnswer: {
      get () {
        return this.$store.state.textTagging2.pybAnswer;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_ANSWER_FIELD, newValue);
      }
    },
    labelAdded: {
      get () {
        return this.$store.state.textTagging2.labelAdded;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_LABEL_ADDED, newValue);
      }
    },
    label: {
      get () {
        return this.$store.state.textTagging2.label;
      },
      set (newValue) {
        this.$store.commit(types.MUTATE_TEXT_TAGGING_2_LABEL, newValue);
      }
    },
    ...mapState({
      tagList: state => state.textTagging2.tagList,
      entityList: state => state.textTagging2.entities.static
    })
  },
  updated () {
  ...
  methods: {
    ...mapMutations({
      'deleteTag': types.MUTATE_TEXT_TAGGING_2_DELETE_TAG,
      'deleteEntity': types.MUTATE_TEXT_TAGGING_2_DELETE_ENTITY
    }),
    getErrors (key) {
      return (this.$store.getters[types.GET_TEXT_TAGGING_2_ERRORS][key] || []).join('\n');
    },
    addTag () {
      this.$store.commit(types.MUTATE_TEXT_TAGGING_2_ADD_TAG);
      this.scrollToEndSelectors.push('#tags');
    },
    addEntity () {
      this.$store.commit(types.MUTATE_TEXT_TAGGING_2_ADD_ENTITY);
